        ...
        if (typeof action === 'undefined' || !action.length || !action[0]) {
            ...
            if (!recovering) {
                ...
                if (this.lexer.showPosition) {
                    // TODO: changed 'yylineno' to 'this.lexer.yylineno' to show
                    // position of first wrong parsed element, not last correct
                    errStr = 'Parse error on line '+(this.lexer.yylineno+1)+":\n"+this.lexer.showPosition()+"\nExpecting "+expected.join(', ') + ", got '" + (this.terminals_[symbol] || symbol)+ "'";
                } else {
                    errStr = 'Parse error on line '+(this.lexer.yylineno+1)+": Unexpected " +
